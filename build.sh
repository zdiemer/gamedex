#!/usr/bin/env bash
# Build the gamedex image and push it to the in-cluster registry (infra/registry). The cluster is multi-node, so
# we ship via the in-cluster registry (infra/registry) rather than side-loading
# into each node's containerd. Re-run after editing anything under
